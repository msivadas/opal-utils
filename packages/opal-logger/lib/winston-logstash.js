...
Logstash.prototype.connect = function () {
  console.log("winston-logstash:  CTOR");
  ...
  this.socket.on('error', function (err) {
    console.log("winston-logstash:  socket error");
    self.connecting = false;
  ...
  this.socket.on('timeout', function() {
    console.log("winston-logstash:  socket timeout");
  ...
  this.socket.on('connect', function () {
    console.log("winston-logstash:  socket connected");
    self.retries = 0;
  });

  this.socket.on('close', function (had_error) {
    self.connected = false;
    console.log("winston-logstash:  socket closed");
    ...
    if (self.max_connect_retries < 0 || self.retries < self.max_connect_retries) {
      if (!self.connecting) {
        setTimeout(function () {
          console.log("winston-logstash: retrying connection");
          self.connect();
...
Logstash.prototype.sendLog = function (message, callback) {
  ...
  //console.log("winston-logstash:  sending message "+message);
